...
- [x] 숫자만 입력 해야 한다.
- [x] 1 이상 31 이하의 숫자만 입력해야 한다.
- [x] 1 이상 31 이하의 숫자가 아닌 경우, "[ERROR] 유효하지 않은 날짜입니다. 다시 입력해 주세요."라는 에러 메시지를 보여 준다.

#### 메뉴와 개수 입력

- [x] 메뉴는 메뉴판에 있는 메뉴만 입력할 수 있다.
- [x] 중복 메뉴를 입력할 수 없다.
- [x] 메뉴의 개수는 1 이상의 숫자만 입력할 수 있다.
- [x] 메뉴의 개수는 합해서 20개를 넘을 수 없다.
- [x] 메뉴와 개수는 가운데 구분자를 통해 구분하여 입력한다.
- [x] 모든 예외 상황에 대해 "[ERROR] 유효하지 않은 주문입니다. 다시 입력해 주세요."라는 에러 메시지를 보여 준다.
...
- [x] 혜택 내역에 따라 혜택 금액을 따로 계산한다.

#### 총혜택 금액 계산

- [x] `총혜택 금액 = 할인 금액의 합계 + 증정 메뉴의 가격` 이다.

#### 할인 후 예상 결제 금액 계산

- [x] `할인 후 예상 결제 금액 = 할인 전 총주문 금액 - 총혜택 금액 + 증정 메뉴의 가격` 이다.
- [x] 변수를 생성하지 않아도 계산이 가능하므로 변수를 생성하지 않는다.
...
- [x] 주문 메뉴마다 개행을 해서 출력한다.
- [x] 메뉴의 이름과 개수를 출력한다.

#### 할인 전 총주문 금액 출력

- [x] 1000원 단위로 콤마를 찍어 구분한다.

#### 증정 메뉴 출력

- [x] 증정 이벤트에 해당하지 않는 경우, 증정 메뉴 "없음"으로 보여 준다.
- [x] 증정 메뉴와 개수를 모두 출력한다.

#### 혜택 내역 출력

- [x] 적용된 이벤트가 하나도 없다면 혜택 내역 "없음"으로 보여 준다.
- [x] 혜택 내역마다 개행을 해서 출력한다.
- [x] 혜택 금액은 '-'를 붙여 마이너스 금액으로 표시한다.
- [x] 혜택 금액은 1000원 단위로 콤마를 찍어 구분한다.

#### 총혜택 금액 출력

- [x] 1000원 단위로 콤마를 찍어 구분한다.
- [x] 앞에 '-'를 붙여 마이너스 금액으로 표시한다.
...
- [x] 이벤트 배지가 부여되지 않으면 이벤트 배지는 "없음"이다.
...
- [x] 테스트코드도 리팩토링을 통해 개선한다.
- [x] 테스트를 위한 코드는 구현 코드에서 분리한다.
- [x] 단위 테스트가 어려운 부분은 클래스 내부가 아닌 외부로 분리한다.
- [x] private 함수를 테스트하려면 클래스 분리를 고려한다.
...
> 이유는 다음과 같습니다. 
> 1. Validator를 Controller에서 싱글톤 방식으로 생성한다고 하더라도 컨트롤러는 애플리케이션이 시작하는 공간이기 때문에 static과 비슷한 시점에 메모리에 올라가고 애플리케이션이 종료되어야만 메모리에서 지워진다는 것을 알게 되었습니다.
...
> 
> 이러한 구조 변경을 통해 자연스럽게 `디미터 법칙`을 지키면서 코드를 작성할 수 있었습니다. Enum에서 정의된 상수를 가져오는 메서드도 단순히 Getter를 사용해 외부로 전달하지 않고 Getter로 값을 전달하는 메서드를 새로 구현했습니다.  
